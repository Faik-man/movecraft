...
public class BlockRail extends Block
{
    ...
    public void onNeighborBlockChange(World par1World, int x, int y, int z, int neighborId)
    {
        if (!par1World.isRemote)
        {
            ...
            if (!par1World.isBlockSolidOnSide(x, y - 1, z, UP))
            {
                var8 = true;
            }

            if (var7 == 2 && !par1World.isBlockSolidOnSide(x + 1, y, z, UP))
            {
                var8 = true;
            }

            if (var7 == 3 && !par1World.isBlockSolidOnSide(x - 1, y, z, UP))
            {
                var8 = true;
            }

            if (var7 == 4 && !par1World.isBlockSolidOnSide(x, y, z - 1, UP))
            {
                var8 = true;
            }

            if (var7 == 5 && !par1World.isBlockSolidOnSide(x, y, z + 1, UP))
            {
                var8 = true;
            }
            ...
            {
                boolean var9 = par1World.isBlockIndirectlyGettingPowered(x, y, z);
                var9 = var9 || this.isNeighborRailPowered(par1World, x, y, z, var6, true, 0) || this.isNeighborRailPowered(par1World, x, y, z, var6, false, 0);
                boolean var10 = false;

                if (var9 && (var6 & 8) == 0)
                {
                    par1World.setBlockMetadataWithNotify(x, y, z, var7 | 8);
                    var10 = true;
                }
                else if (!var9 && (var6 & 8) != 0)
                {
                    par1World.setBlockMetadataWithNotify(x, y, z, var7);
                    var10 = true;
                }
            ...
            else if (neighborId > 0 && Block.blocksList[neighborId].canProvidePower() && !this.isPowered && RailLogic.getNAdjacentTracks(new RailLogic(this, par1World, x, y, z)) == 3)
            {
                this.refreshTrackShape(par1World, x, y, z, false);
            }
        }
    }
